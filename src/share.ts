...
const showPreview=(url:string)=>{
  ...
  layer.innerHTML=`<div class="share-preview-card"><h2>캡처가 완성됐어요</h2><p>이미지를 길게 누른 뒤 이미지 공유를 선택하면 카카오톡으로 보낼 수 있어요.</p><img src="${url}" alt="완성된 스쿼드 캡처"><button class="share-preview-open" type="button">이미지만 크게 보기</button><button class="share-preview-close" type="button">닫기</button></div>`;
  const close=()=>{URL.revokeObjectURL(url);layer.remove()};
  layer.querySelector('.share-preview-close')?.addEventListener('click',close);
  layer.querySelector('.share-preview-open')?.addEventListener('click',()=>layer.classList.add('is-image-only'));
